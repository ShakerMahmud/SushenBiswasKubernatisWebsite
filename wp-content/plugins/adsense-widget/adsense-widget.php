...
class stf_adsense extends WP_Widget {
  ...
  function widget( $args, $instance ) {
    ...
    if( empty( $publisher_id ) && current_user_can( 'administrator' ) ) { 
			echo '<div class="adsense-warning">Publisher Id is blank. Please add Publisher Id in <a href="https://metinsaylan.com/wordpress/plugins/adsense-widget/">Adsense Widget</a> settings page.</div>';
		} else {
        ...
        echo '<div class="adsense-wrapper">';
    ...
    function form( $instance ) {
      ...
      if( empty( $adsense_id ) ) {
		...
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title :', 'adsense-widget'); ?> <small><a href="https://metinsaylan.com/wordpress/plugins/adsense-widget/help/#title" target="_blank" rel="external">(?)</a></small> <input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></label><br />

    <p><label for="<?php echo $this->get_field_id('adsense_id'); ?>"><?php _e('Publisher Id :', 'adsense-widget'); ?> <small><a href="https://metinsaylan.com/wordpress/plugins/adsense-widget/help/#adsense-id" target="_blank" rel="external">(?)</a></small> <input class="widefat" id="<?php echo $this->get_field_id('adsense_id'); ?>" name="<?php echo $this->get_field_name('adsense_id'); ?>" type="text" value="<?php echo $adsense_id; ?>" /></label><br />

    <p><label for="<?php echo $this->get_field_id('format'); ?>"><?php _e('Ad format:', 'adsense-widget'); ?> <small><a href="https://metinsaylan.com/wordpress/plugins/adsense-widget/help/#ad-format" target="_blank" rel="external">(?)</a></small> <select name="<?php echo $this->get_field_name('format'); ?>" id="<?php echo $this->get_field_id('format'); ?>" ><?php 
    ...
    <p><label for="<?php echo $this->get_field_id('responsive'); ?>"><input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id('responsive'); ?>" name="<?php echo $this->get_field_name('responsive'); ?>" value="on" <?php checked( 'on', $responsive ) ?> /> Responsive</label></p>
		...
		<p><label for="<?php echo $this->get_field_id('slot'); ?>"><?php _e('Slot ID (Optional):', 'adsense-widget'); ?> <small><a href="https://metinsaylan.com/wordpress/plugins/adsense-widget/help/#slot-id" target="_blank" rel="external">(?)</a></small> <input class="widefat" id="<?php echo $this->get_field_id('slot'); ?>" name="<?php echo $this->get_field_name('slot'); ?>" type="text" value="<?php echo $slot; ?>" /></label><br />
    <small>Slot id for the ad you created.( Eg. 1234567890 )</small><br />
    
    <p><label for="<?php echo $this->get_field_id('sizes'); ?>"><?php _e('Sizing:', 'adsense-widget'); ?> <small><a href="https://metinsaylan.com/wordpress/plugins/adsense-widget/sizing-options/" target="_blank" rel="external">(?)</a></small> <input class="widefat" id="<?php echo $this->get_field_id('sizes'); ?>" name="<?php echo $this->get_field_name('sizes'); ?>" type="text" value="<?php echo $sizes; ?>" /></label><br />
    <small>Sizing CSS classes.</small><br />
    
    <p><label for="<?php echo $this->get_field_id('layout_key'); ?>"><?php _e('Layout Key (In Feed Ads):', 'adsense-widget'); ?> <small><a href="https://metinsaylan.com/wordpress/plugins/adsense-widget/help/#layout-key" target="_blank" rel="external">(?)</a></small> <input class="widefat" id="<?php echo $this->get_field_id('layout_key'); ?>" name="<?php echo $this->get_field_name('layout_key'); ?>" type="text" value="<?php echo $layout_key; ?>" /></label><br />
