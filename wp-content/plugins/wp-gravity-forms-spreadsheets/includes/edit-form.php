...
if( !class_exists( 'vxg_googlesheets_form' ) ) {
class vxg_googlesheets_form extends vxg_googlesheets{
...
$feed=get_option($this->id.'_lists',array()); 
  ...
  /**
  * Replace the Gravity Forms form choices with remotely-pulled Google Sheets picklist options.
...
if(isset($objects[$object])){
 ...
 public function fields_html($id,$object,$refresh=false){
  ...
  if(!is_array($fields)){
      $msg= empty($fields) ? __('Fields Not Found','gravity-forms-googlesheets-crm') : $fields;
  $str = '<div class="field_group_'.$this->id.'"><strong>'.__('Error:', 'gravity-forms-googlesheets-crm').'</strong> '.$msg.'</div>';
  ...
  */
  ...
  if(empty($value['value'])) { continue; }
        $default_html = !empty($value['default']) ?  '<strong class="default"> '.esc_html__('(Default)', 'gravity-forms-googlesheets-crm').'</strong>' : '';
  ...
  */
  ...
  <tr>
  <th scope='col' class='crm_col_heading'>" . __("Pickist Field", 'gravity-forms-googlesheets-crm') . "</th>
  <th scope='col' class='crm_col_heading'>" . __("Form Fields", 'gravity-forms-googlesheets-crm') . "</th>
  </tr>
  </thead>
  <tbody>".$str."
  </tbody>
  ...
  $str='<div><b>'.__('No Options Found','gravity-forms-googlesheets-crm').'</b></div>';    
  }
  /// $str = str_replace(array("\n", "\t", "\r"), '', str_replace("'", "\'", $str));
...
//  toggle_choices(check_box);  
  ...
  */
  ...
  $tooltips['vx_map_live_'.$this->id] = sprintf(__('%sUpdate from Google Sheets %s If you update a picklist in Google Sheets, the modifications will be added to your form without having to edit the field Choices in Gravity Forms. You will not be able to edit the Choices in Gravity Forms, you can only update them in Google Sheets. The order of Choices as well as the default values are determined by the Google Sheets picklist field settings.', 'gravity-forms-googlesheets-crm'), '<h6>','</h6>');
  $tooltips['vx_map_once_'.$this->id] = sprintf(__('%sUpdate from Google Sheets %s Field Choices will not be updated live from Google Sheets and are editable in Gravity Forms. If you make an edit in Google Sheets, it will not be updated in your form.', 'gravity-forms-googlesheets-crm'), '<h6>','</h6>');
  return $tooltips;
  }
  /**
  ...
  */
  ...
  <input type="checkbox" autocomplete="off" class="<?php echo $this->id ?>_map_enabled crm_map_enabled" id="vx_check_<?php echo $this->id ?>" name="crm_map_enabled" value="<?php echo $this->id ?>" /> <?php _e("Enable Google Sheets Field Mapping?", 'gravity-forms-googlesheets-crm'); ?>
  </label>
  
  <div id="<?php echo $this->id ?>_map_ui" style="display: none;">
  
  <label for="<?php echo $this->id ?>_map_type_live">
  <input type="radio" class="<?php echo $this->id ?>_map_type crm_map_type" id="<?php echo $this->id ?>_map_type_live" name="<?php echo $this->id ?>_map_type" value="live" /> <?php _e("Live Remote Field Mapping ", 'gravity-forms-googlesheets-crm'); gform_tooltip("vx_map_live_".$this->id); ?>
  <span class="howto" style="padding-left:1.25em;"><?php _e("Field Choices will be synced from Google Sheets picklist values.", 'gravity-forms-googlesheets-crm'); ?></span>
  </label>
  <label for="<?php echo $this->id ?>_map_type_once">
  <input type="radio" class="<?php echo $this->id ?>_map_type crm_map_type" id="<?php echo $this->id ?>_map_type_once" name="<?php echo $this->id ?>_map_type" value="once" /> <?php _e("Only Populate Choices ", 'gravity-forms-googlesheets-crm'); gform_tooltip("vx_map_once_".$this->id) ?>
  <span class="howto" style="padding-left:1.25em;"><?php _e("Field Choices will not be updated live and are editable.", 'gravity-forms-googlesheets-crm'); ?></span>
  </label>
    <label for="crm_accounts_list" style="width: 100px;" class="inline"><?php _e("Choose Account", 'gravity-forms-googlesheets-crm'); ?></label>
 ...
 public function object_area($id,$object){
     ...
     ?>
       <label for="crm_object_list" style="width: 100px;" class="inline"><?php _e("Choose Object", 'gravity-forms-googlesheets-crm'); ?></label>
  <?php
  if(!is_array($lists)) {
  echo __("Could not load Google Sheets objects.", 'gravity-forms-googlesheets-crm');
  } else { ?>
  ...
  <option value=""><?php _e("Select a Google Sheets Object", 'gravity-forms-googlesheets-crm'); ?></option>
  <?php
